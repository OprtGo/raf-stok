...
export default function ProductByShortCode({ params }: { params: Promise<{ shortCode: string }> }) {
      ...
      <div style={styles.card}>
        ...
        <div style={styles.imageSection}>
          <div style={styles.mainImageWrapper} onClick={() => setTamEkranResim(seciliResim)}>
            {seciliResim ? (
              <img src={seciliResim} alt={urun.name} style={styles.mainImage} />
            ) : (
              <p style={{color: '#999'}}>Resim Yok</p>
            )}
          </div>
          {urun.imageUrls && urun.imageUrls.length > 1 && (
            <div style={styles.thumbnailList}>
              {urun.imageUrls.map((url: string, index: number) => (
                <div 
                  key={index} 
                  style={{
                    ...styles.thumbnail,
                    border: seciliResim === url ? '2px solid #2563EB' : '2px solid transparent'
                  }}
                  onClick={() => setSeciliResim(url)}
                >
                  <img src={url} style={{width: '100%', height: '100%', objectFit: 'cover'}} alt="thumb" />
                </div>
              ))}
            </div>
          )}
        ...
        <div style={styles.infoSection}>
          ...
          <div style={styles.specList}>
            <div style={styles.specRow}>
              <span>Kutu</span>
              <span style={styles.specValue}>{urun.boxStatus}</span>
            </div>
            <div style={styles.specRow}>
              <span>Garanti</span>
              <span style={styles.specValue}>{urun.warrantyStatus}</span>
            </div>
          </div>

          {urun.description && (
            <div style={styles.descSection}>
              <p style={styles.descTitle}>Açıklama</p>
              <p style={styles.descText}>{urun.description}</p>
            </div>
          )}

          <button onClick={handleShare} style={styles.shareBigButton}>
            <FiShare2 size={20} /> {paylasildi ? "Kopyalandı!" : "Paylaş"}
          </button>
        </div>
      </div>
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.editCard}>
            <div style={styles.modalHeader}>
              ...
              <button onClick={() => setEditModu(false)} style={{border: 'none', background: 'none', fontSize: 20, cursor: 'pointer'}}>
                <FiX />
              </button>
            </div>
